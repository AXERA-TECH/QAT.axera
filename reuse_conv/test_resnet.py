"""加载 train_resnet.py 产物做分段推理评测(torch 2.10,utils 统一 API)。

说明:
  1. capture 与训练侧一致(动态 batch,保证 prepared state_dict 键对齐);
  2. ORT 输入名动态获取(不硬编码);
  3. 数据 fake_data;评测结果补打印(原版收集后未打印);
  4. 加载 reuse_conv/ 下的产物。

运行(先跑 train_resnet.py):
  cd /home/heqi/project-qat/QAT.axera && PYTHONPATH=. CUDA_VISIBLE_DEVICES=<空卡> \
    <env>/bin/python reuse_conv/test_resnet.py
"""
import copy
...
from utils.train_utils import evaluate, evaluate_np, imagenet_data_loaders
...
def test():
    ...
    # 捕获与训练侧一致(batch 动态,example 给 >=2 规避 0/1 特化)
