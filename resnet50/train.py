"""resnet50 QAT 训练 —— utils 统一 API 版(torch 2.6 / 2.10 同一份代码)。

版本差异全部由 utils 内部消化(capture/dynamo_export/export_float_reference),
...
运行(qat-dev):
  cd /home/heqi/project-qat/QAT.axera && \
    ...
    <env>/bin/python resnet50/train.py --data fake --config ./resnet50/config_4w4f_all.json
"""
import argparse
import os
