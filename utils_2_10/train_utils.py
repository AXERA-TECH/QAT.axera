...
def dynamo_export(model, inputs, onnx_path):
    # torch2.9+ 默认 optimize=True,onnxscript 新版常量折叠会把
    # 「int8 权重 + DequantizeLinear」折成 float 权重(量化信息全丢),必须关掉;
    # 常量折叠交给下游 simplify_and_fix_4bit_dtype 的 gs.fold_constants + slim(不折 QDQ)
