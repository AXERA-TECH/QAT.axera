...
from torchao.quantization.pt2e.quantizer import QuantizationSpec, Quantizer, DerivedQuantizationSpec
from torchao.quantization.pt2e.quantizer.utils import get_module_name_filter as _get_module_name_filter
...
def _is_bn_num_batches_tracked_source(node: torch.fx.Node) -> bool:
    ...
    torch 2.10 的 export 图不携带 source_fn_stack 元数据,按 buffer 名识别
    (num_batches_tracked 是 BN 专有 buffer)。
    """
    while (
