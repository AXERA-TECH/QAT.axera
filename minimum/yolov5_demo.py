"""多输入 BEV 小网络 QAT 导出(torch 2.10,utils 统一 API)。

覆盖算子:ConvTranspose、cat、grid_sample、permute/reshape(共享观察器传播)、
多层 Linear——检验 axquant 里 convtranspose/concat/gridsample/linear
注解器与 propagate_annotation 在 2.10 下的适配。

运行:
  cd /home/heqi/project-qat/QAT.axera && PYTHONPATH=. CUDA_VISIBLE_DEVICES=<空卡> \
    <env>/bin/python minimum/yolov5_demo.py
"""
import torch
...
# export float_model(与 QAT 导出同路径 dynamo_export)
