...
class Model(torch.nn.Module):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        from ultralytics.utils.quantizer import AXQuantizer, get_quantization_config
        ...
        # for i in range(3):
        #     quantizer.set_module_name('model.'+str(i), get_quantization_config(is_qat=True))
        self.model = self.model.to("cpu")
        inputs = torch.rand(1, 3, 640, 640).to("cpu")
        exported_model = torch.export.export_for_training(self.model, (inputs,)).module()
        ...
        onnx_program = torch.onnx.export(quantized_model, (inputs.to("cuda"),), dynamo=True)
        onnx_program.optimize()
