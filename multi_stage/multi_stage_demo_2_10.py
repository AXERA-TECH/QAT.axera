...
相对 2.6 版的改动:
  ...
  3. 数据:机器上没有 ImageNet 训练集 → CIFAR-10(10 类 fc,种子与
     resnet50_2_10 训练一致),checkpoint 用全 epoch QAT 产物
...
if __name__ == "__main__":
    ...
    # quantizer
    global_config, regional_configs = load_config("./resnet50_2_10/config.json")
    quantizer = AXQuantizer("./resnet50_2_10/config.json")

    # float model(10 类 fc,种子与 resnet50_2_10/train.py 一致以对上 checkpoint)
    ...
    prepared_model.load_state_dict(
        torch.load("./resnet50_2_10/checkpoint/last_checkpoint_2_10.pth", weights_only=True))
