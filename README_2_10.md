...
> 状态：**P0–P4 全部完成**（2026-07-07 ~ 07-10）。
> 双轨共存：本仓库所有 2.6 原件未动，2.10 实现一律以 `*_2_10` / `utils_2_10` 命名平行存在。
...
PYTHONPATH=. CUDA_VISIBLE_DEVICES=<空卡> \
  python minimum/minimum_demo_2_10.py
...
|------------------|-------------|------|
| `utils/` | **`utils_2_10/`** | 核心迁移:量化器全家迁 torchao、注解器 2.10 适配、导出/后处理修复(见下表) |
| `requirements.txt` | `requirements_2_10.txt` | 版本选型依据见 env.md |
| `minimum/minimum_demo.py` | `minimum/minimum_demo_2_10.py` | 与 2.6 金标准 sim 16/16 全项一致 |
| `minimum/yolov5_demo.py` | `minimum/yolov5_demo_2_10.py` | grid_sample/ConvTranspose/cat/linear 全绿 |
| `resnet50/` | **`resnet50_2_10/`** | train/test/cross_export 均为**双环境单脚本**(按 torch 版本自动切实现);5 份量化配置副本 |
| `multi_stage/*.py` | `multi_stage/*_2_10.py` | 切子图点自动定位(见"必知差异"第 7 条);3≈5 精度一分不差 |
| `reuse_conv/*.py` | `reuse_conv/*_2_10.py` | 复用 BN hack 已有 2.10 实现;含逐 bit fixture 回归 |
| `test_clamp/*_demo.py` | `test_clamp/*_demo_2_10.py` | 4/4 与 2.6 数值输出逐字符一致 |
| `yolov5/train.py` | `yolov5/train_2_10.py` | 外部 yolov5 仓库参考补丁(不在本仓库运行) |
...
1. **PT2E 必须走 torchao**：torch.ao 的 `convert_pt2e` 在 2.10 已坏（`KeyError: 'source_fn_stack'`）；
   且 torch.ao 与 torchao 的 QuantizationSpec 类型不互通，量化器需整体换命名空间（utils_2_10 已完成）。
